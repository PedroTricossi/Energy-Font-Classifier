...
def score_fn(model: pl.LightningModule, x: torch.Tensor, y: torch.Tensor = None, score: str = "mass"):
    if score == "px":
        return -model.cnn(x, y).detach().cpu()
    elif score == "py":
        return torch.nn.functional.softmax(model.cnn.classify(x)).max(1)[0].detach().cpu()
    elif score == "mass":
